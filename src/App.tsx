...
export default function App() {
  ...
  const handleResetToExtent = () => {
    ...
    setMeasureMode("none");
    setMeasurePoints([]);
    // Reset layer visibility to only active district boundary and default opacity
    setLayers((prev) => prev.map((l) => {
      const lowerName = l.name.toLowerCase();
      const isDistrict = lowerName === "district-boundary" || lowerName === "district_boundary";
      return {
        ...l,
        visible: isDistrict,
        opacity: l.type === "polygon" ? 1.0 : 0.9
      };
    }));
  };
          ...
          <div className="absolute inset-x-0 inset-y-0 bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center z-50 p-6 select-none font-sans">
            <div className="bg-slate-800 border border-slate-700/80 p-8 rounded-2xl shadow-2xl flex flex-col items-center max-w-sm text-center">
              ...
              <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                Connecting securely to database. Downloading geographical boundaries, river streams, and villages of <span className="text-indigo-400 font-semibold">Pithoragarh</span>...
              </p>
