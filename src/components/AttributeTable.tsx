...
export default function AttributeTable({
  ...
  if (isCollapsed) {
    return (
      <div className={`w-12 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col items-center pt-16 pb-4 shrink-0 transition-all duration-300 ${disabled ? "pointer-events-none opacity-40 select-none" : ""}`}>
        <button
          onClick={() => !disabled && setIsCollapsed(false)}
          disabled={disabled}
          title={disabled ? "Authentication required" : "Open Attribute Table"}
          className={`p-2 text-slate-600 dark:text-slate-300 rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 shadow-sm transition duration-150 mt-4 mb-8 ${disabled ? "cursor-not-allowed opacity-50" : "hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-800 cursor-pointer"}`}
        >
  ...
  return (
    <div className={`w-[450px] border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 flex flex-col h-full shrink-0 shadow-sm font-sans transition-all duration-300 ${disabled ? "pointer-events-none opacity-40 select-none" : ""}`}>
            ...
            {currentLayer && (
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full text-xs pl-9 pr-4 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition"
                />
              </div>
            )}
          </>
      ...
      {/* Stats Counter Bar */}
      {currentLayer && (
        <div className="px-4 py-2 bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 flex justify-between text-[11px] items-center text-slate-500 dark:text-slate-400 font-medium font-mono">
      ...
      <div className="flex-1 overflow-auto min-h-0 bg-white dark:bg-slate-900 relative">
        {!currentLayer ? (
          ...
          <table className="w-full text-left text-xs border-collapse font-sans">
            <thead>
              <tr className="bg-slate-100/60 dark:bg-slate-800/60 sticky top-0 border-b border-slate-200 dark:border-slate-800 shadow-sm z-10 text-slate-600 dark:text-slate-300 font-bold leading-tight select-none">
                <th className="py-2.5 px-3 whitespace-nowrap text-center text-[10px] w-12 border-r border-slate-200 dark:border-slate-800 uppercase tracking-widest bg-slate-100 dark:bg-slate-800">#</th>
                <th className="py-2.5 px-3 whitespace-nowrap text-[10px] w-24 border-r border-slate-200 dark:border-slate-800 uppercase tracking-widest bg-slate-100 dark:bg-slate-800">Locate</th>
                {columnKeys.map((key) => (
                  <th key={key} className="py-2.5 px-3 border-r border-slate-200 dark:border-slate-800 uppercase tracking-wider font-mono text-[10px] bg-slate-100 dark:bg-slate-800">
                    {key}
                  </th>
                ))}
              </tr>
                ...
                return (
                  <tr
                    ...
                    className={`group cursor-pointer text-slate-700 dark:text-slate-300 transition duration-150 ${
                      isSelected 
                        ? "bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-900 dark:text-indigo-200 border-l-2 border-l-indigo-600 dark:border-l-indigo-400 border-b border-indigo-200 dark:border-indigo-900/50" 
                        : "hover:bg-slate-50 dark:hover:bg-slate-800/60 border-l-2 border-l-transparent"
                    }`}
                  >
                    <td className="py-2.5 px-3 text-center border-r border-slate-100 dark:border-slate-800 font-mono text-[10px] text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-850/50">
                      {index + 1}
                    </td>
        ...
        <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800/50 border-t border-b border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs font-semibold select-none text-slate-600 dark:text-slate-300 shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40 disabled:hover:bg-white dark:disabled:hover:bg-slate-800 transition cursor-pointer text-[10px]"
            >
              Prev
            </button>
      ...
      <div className="border-t border-slate-200 dark:border-slate-800 bg-slate-900 text-slate-100 shadow-inner select-none font-sans flex flex-col shrink-0">
        <button
