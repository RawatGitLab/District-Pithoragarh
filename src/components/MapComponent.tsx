...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Pithoragarh, Uttarakhand, India (True center: 30.12, 80.38)
  ...
  // Map Controls: Pan to Pithoragarh bounds safely
  const handleZoomToDistrict = () => {
    ...
    // Fallback static center of Pithoragarh
    map.setView([30.12, 80.38], 9, { animate: true });
  };
    ...
    <div className="relative flex-1 bg-slate-100 flex flex-col h-full min-w-0">
      ...
      <div className="absolute top-4 left-4 z-[1001] w-80 font-sans">
        <div className="relative flex items-center bg-white/95 dark:bg-slate-900/95 backdrop-blur border border-slate-200 dark:border-slate-800 rounded-lg shadow-md transition-shadow duration-200 focus-within:shadow-lg focus-within:border-indigo-400 dark:focus-within:border-indigo-500">
          ...
          <input
            type="text"
            className="w-full text-xs pl-9 pr-8 py-2.5 bg-transparent rounded-lg text-slate-700 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 font-semibold focus:outline-none"
            ...
            {filteredSearchFeatures.map((feat) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left px-3.5 py-2.5 hover:bg-indigo-50/70 dark:hover:bg-slate-800 active:bg-indigo-100 dark:active:bg-slate-700 text-xs text-slate-700 dark:text-slate-200 font-medium transition-colors flex items-center justify-between gap-1 border-none bg-transparent cursor-pointer"
                >
                  ...
                  <span className="text-[9px] uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-400 font-bold px-1.5 py-0.5 rounded font-mono shrink-0">
                    {layerName.replace("USN-", "").replace("Pithoragarh-", "").replace("-Boundary", "")}
                  </span>
                </button>
              );
            })}
